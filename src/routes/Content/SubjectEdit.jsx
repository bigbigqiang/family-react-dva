...
@Form.create()
export default class SubjectEdit extends PureComponent {
    ...
    generateStaticBookList(bookCode) {
        ...
        selected.map(item => {
            ...
            let tags2 = _.get(item, 'tag', '').split(',');
            ...
            // parent.book.summary
            let book = `
                <a class='book book_${item.id}' href="ellafamily://#LinkHolder?book_code=${item.bookCode}&method=ella.book.getBookByCode">
                    ${bookCover}
                    <div class="meta">${bookTitle}${bookTags}${bookAuthor}${bookGrade}</div>
                </a>
            `;
    ...
    render() {
        ...
        const { subjects: { subjectList, editContent, bookList } } = this.props;
        ...
        return (
            <div className={['clf']} style={{ minWidth: '858px' }}>
                <Row>
                    <Col>
                        <Link to="/content/subjects?type=back">
                                    ...
                                    () => {
                                        ...
                                        let inter = setInterval(function () {
                                            ...
                                            if (all.length) {
                                                ...
                                                // 2. bookCode转字符串之前，生成静态页面需要的 图书推荐列表内容
                                                var bookHTML = that.generateStaticBookList(values.bookCode.split(','));

                                                // values.topicContent = encodeURIComponent(values.topicContent.replace(/\"/g, "'").replace(/\%/g, "%25"));

                                                // console.log(values.topicContent)
                                                ...
                                                ghostHTML = ghostHTML
                                                    ...
                                                    `)
                                                    .replace('#ContentHolder', `<div id='ContentHolder'>${values.topicContent}</div>`)
                                                    .replace('#BookListHolder', `<div id='BookListHolder'>${bookHTML}</div>`)
                                                    .replace(/\"/g, "'");
